...
import { post1, post1_tr } from './post-1'
import { post2, post2_tr } from './post-2'
import { post3, post3_tr } from './post-3'
...
export const seed = async ({
  ...
  await payload.update({
    ...
    id: post1Doc.id,
    data: JSON.parse(
      JSON.stringify({ ...post1_tr, categories: [technologyCategory.id] })
        .replace(/"\{\{IMAGE_1\}\}"/g, String(image1ID))
        .replace(/"\{\{IMAGE_2\}\}"/g, String(image2ID))
        .replace(/"\{\{AUTHOR\}\}"/g, String(demoAuthorID)),
    ),
  ...
  await payload.update({
    ...
    id: post2Doc.id,
    data: JSON.parse(
      JSON.stringify({ ...post2_tr, categories: [newsCategory.id] })
        .replace(/"\{\{IMAGE_1\}\}"/g, String(image2ID))
        .replace(/"\{\{IMAGE_2\}\}"/g, String(image3ID))
        .replace(/"\{\{AUTHOR\}\}"/g, String(demoAuthorID)),
    ),
  ...
  await payload.update({
    ...
    id: post3Doc.id,
    data: JSON.parse(
      JSON.stringify({ ...post3_tr, categories: [financeCategory.id] })
        .replace(/"\{\{IMAGE_1\}\}"/g, String(image3ID))
        .replace(/"\{\{IMAGE_2\}\}"/g, String(image1ID))
        .replace(/"\{\{AUTHOR\}\}"/g, String(demoAuthorID)),
    ),
